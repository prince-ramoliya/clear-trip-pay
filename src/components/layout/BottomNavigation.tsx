...
const navItems = [
  ...
  { id: 'settlements', label: 'Settle', icon: Wallet },
];

export function BottomNavigation({ currentView, onViewChange, onAddExpense, hasTripSelected }: BottomNavigationProps) {
      ...
      <div className="flex items-center justify-around h-16 px-2">
        {navItems.map(item => (
          <button
            ...
            className={cn(
              "flex flex-col items-center justify-center flex-1 h-full py-1 px-2 transition-colors min-w-0",
              currentView === item.id 
                ? "text-primary" 
        ...
        <button
          ...
          className="flex flex-col items-center justify-center flex-1 h-full py-1 px-2"
        >

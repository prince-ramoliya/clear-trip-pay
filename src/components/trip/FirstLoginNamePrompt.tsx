...
export function FirstLoginNamePrompt({ userId }: FirstLoginNamePromptProps) {
  ...
  // Check localStorage to see if we've already shown the prompt
  const storageKey = `first_login_prompt_shown_${userId}`;

  useEffect(() => {
    if (userId && profile && isAutoGeneratedName) {
  ...
  }, [userId, profile, isAutoGeneratedName, storageKey]);
  ...
  if (!isAutoGeneratedName) return null;
